...
int Paint::init(HWND hWND) {
	...
	g_pD3D->CreateDeviceEx(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, hWND, D3DCREATE_HARDWARE_VERTEXPROCESSING, &g_d3dpp, 0, &g_pd3dDevice);

	D3DXCreateFont(g_pd3dDevice, 12, 0, FW_BOLD, 1, false, DEFAULT_CHARSET, OUT_DEVICE_PRECIS, ANTIALIASED_QUALITY, DEFAULT_PITCH, L"Comic Sans", &font);
...
int Paint::render() {
    ...
    if (true || TargetHWND == GetForegroundWindow()) {
        // Main Program Runs here
        hack.Update(width, height);
    }

	g_pd3dDevice->EndScene();
	g_pd3dDevice->PresentEx(0, 0, 0, 0, 0);
...
void Paint::DrawCircle(int x, int y, int radius, int numSides, DWORD color) {
	ID3DXLine* LineL;
	D3DXCreateLine(g_pd3dDevice, &LineL);
